...
select avg(students_marks), student_batch from ineuron_student group by student_batch
select student_batch, count(*) from ineuron_student group by student_batch


/* Analytical based windowing function */

select student_name from ineuron_student order by students_marks limit 1

select student_name, max(students_marks) from ineuron_student
select student_name from ineuron_student where  students_marks in (select max(students_marks) from ineuron_student)

select * from ineuron_student

select student_name, students_marks from ineuron_student where 
student_batch = "fsda" order by  students_marks desc limit 1,1

select student_name, students_marks from ineuron_student where 
student_batch = "fsda" order by  students_marks desc limit 2,3

select student_name, students_marks from ineuron_student where 
student_batch = "fsda" order by  students_marks desc
...
row_number() over(order by students_marks ) as 'row_number' from ineuron_student

select student_id , student_batch , student_stream,students_marks ,
row_number() over(partition by student_batch order by students_marks ) as 'row_number' from ineuron_student
...
from ineuron_student) as test where row_rank = 1
